...
Protected Module AutolayoutExtensions
	...
	#tag Method, Flags = &h0, CompatibilityFlags = (TargetDesktop and (Target64Bit))
		Sub AddConstraint(extends control as RectControl, constraint as SOSLayoutConstraint)
		  // adds an SOSLayoutConstraint to the DesktopUIControl
		  #if TargetMacOS
		    // - (void)addConstraint:(NSLayoutConstraint *)constraint;
		    Declare Sub addConstraint Lib "Foundation" Selector "addConstraint:" ( obj As integer , constraint As Ptr )
	...
	#tag Method, Flags = &h0, CompatibilityFlags = (TargetDesktop and (Target64Bit))
		Sub AddConstraint(extends screen as Window, constraint as SOSLayoutConstraint)
		  // adds an SOSLayoutConstraint to the DesktopWindow
		  #if TargetMacOS
	...
	#tag Method, Flags = &h0, CompatibilityFlags = (TargetDesktop and (Target64Bit))
		Sub AddConstraints(extends view as RectControl, constraints() as SOSLayoutConstraint, priority as Double = 1000.0)
		  // adds an array of SOSLayoutConstraints to the DesktopUIControl with a specific priority
		  // throttle to valid values
		  ThrottlePriority(priority)
	...
	#tag Method, Flags = &h0, CompatibilityFlags = (TargetDesktop and (Target64Bit))
		Sub AddConstraints(extends view as Window, constraints() as SOSLayoutConstraint, priority as Double = 1000.0)
		  // adds an array of SOSLayoutConstraints to the DesktopWindow with a particular priority
	...
	#tag Method, Flags = &h0, CompatibilityFlags = (TargetConsole and (Target32Bit or Target64Bit)) or  (TargetWeb and (Target32Bit or Target64Bit)) or  (TargetDesktop and (Target32Bit or Target64Bit))
		Sub AlignToLabel(extends ctl as RectControl, label as Label)
		  // Creates leading constraint and either a baseline or top constraint between the controls and the label
		  ...
		  #If TargetMacOS
		    ...
		    Case IsA PushButton, IsA Combobox, IsA PopupMenu
		      ctl.FirstBaselineAnchor.ConstraintEqualToAnchor(label.FirstBaselineAnchor, 2).Active = True
		      
		    Case IsA RadioButton, IsA CheckBox, IsA Label
		      ctl.FirstBaselineAnchor.ConstraintEqualToAnchor(label.FirstBaselineAnchor).Active = True
		    ...
		    Declare Sub setAlignment Lib "Foundation" Selector "setAlignment:" (obj As integer, value As Integer)
		    If SystemIsRTL Then
		      setAlignment(label.Handle, 1)
		    Else
		      setAlignment(label.Handle, 3) 
	...
	#tag Method, Flags = &h0, CompatibilityFlags = (TargetDesktop and (Target64Bit))
		Function CompressionResistancePriorityForAxis(Extends item as ContainerControl, axis as SOSConstraintKit.Axis) As Double
		  ...
		  #If TargetMacOS
		    ...
		    Declare Function contentCompressionResistancePriorityForOrientation Lib "Foundation" Selector "contentCompressionResistancePriorityForOrientation:" ( obj As integer , orientation As Integer ) As Single
	...
	#tag Method, Flags = &h0, CompatibilityFlags = (TargetDesktop and (Target64Bit))
		Sub CompressionResistancePriorityForAxis(Extends item as ContainerControl, axis as SOSConstraintKit.Axis, assigns priority as double)
		  ...
		  #If TargetMacOS
		    ...
		    Declare Sub setContentCompressionResistancePriority_forOrientation Lib "Foundation" Selector "setContentCompressionResistancePriority:forOrientation:" ( obj As integer , priority As Single , orientation As Integer )
	...
	#tag Method, Flags = &h0, CompatibilityFlags = (TargetDesktop and (Target64Bit))
		Function CompressionResistancePriorityForAxis(Extends item as RectControl, axis as SOSConstraintKit.Axis) As Double
		  // Gets the Compression Resistance priority for the specified Axis
		  #If TargetMacOS
		    // - (NSLayoutPriority)contentCompressionResistancePriorityForOrientation:(NSLayoutConstraintOrientation)orientation;
		    Declare Function contentCompressionResistancePriorityForOrientation Lib "Foundation" Selector "contentCompressionResistancePriorityForOrientation:" ( obj As integer , orientation As Integer ) As Single
	...
	#tag Method, Flags = &h0, CompatibilityFlags = (TargetDesktop and (Target64Bit))
		Sub CompressionResistancePriorityForAxis(Extends item as RectControl, axis as SOSConstraintKit.Axis, assigns priority as double)
		  // Sets the Compression Resistance priority for the specified Axis
	...
	#tag Method, Flags = &h0, CompatibilityFlags = (TargetDesktop and (Target64Bit))
		Function ConstraintForControlAttribute(extends view as Window, ctl as RectControl, attr as SOSLayoutConstraint.LayoutAttributes) As SOSLayoutConstraint
		  // Gets a constraint for a particular attribute 
		  #if TargetMacOS
		    Dim ca() As SOSLayoutConstraint = view.Constraints
		    
		    For i As Integer = 0 To UBound(ca)
		      If ca(i).FirstItem = Ptr(ctl.Handle) Then
		        If ca(i).FirstAttribute = attr Then
		          Return ca(i)
		        End If
		      End If
	...
	#tag Method, Flags = &h0, CompatibilityFlags = (TargetDesktop and (Target64Bit))
		Function ConstraintMatchingIntrinsicHeight(extends view as RectControl) As SOSLayoutConstraint
		  // Creates an intrinsic height constraint for the specified control
		  #if TargetMacOS
		    Dim sz As size = view.IntrinsicContentSize
		    
		    Return New SOSLayoutConstraint(Ptr(view.Handle), SOSLayoutConstraint.LayoutAttributes.Height, SOSLayoutConstraint.relations.Equal, sz.Width)
	...
	#tag Method, Flags = &h0, CompatibilityFlags = (TargetDesktop and (Target64Bit))
		Function ConstraintMatchingIntrinsicWidth(extends view as RectControl) As SOSLayoutConstraint
		  // Creates an intrinsic width constraint for the specified control
		  #if TargetMacOS
		    Dim sz As size = view.IntrinsicContentSize
		    
		    Return New SOSLayoutConstraint(Ptr(view.Handle), SOSLayoutConstraint.LayoutAttributes.Width, SOSLayoutConstraint.relations.Equal, sz.Width)
	...
	#tag Method, Flags = &h0, CompatibilityFlags = (TargetDesktop and (Target64Bit))
		Function Constraints(extends control as RectControl) As SOSLayoutConstraint()
		  // Returns all of the constraints attached to the specified DesktopUIControl
		  #If TargetMacOS
		    // @property(nonatomic, readonly) NSArray<__kindof NSLayoutConstraint *> *constraints;
		    Declare Function getConstraints Lib "Foundation" Selector "constraints" (obj As integer) As Ptr
	...
	#tag Method, Flags = &h0, CompatibilityFlags = (TargetDesktop and (Target64Bit))
		Function Constraints(extends screen as Window) As SOSLayoutConstraint()
		  // Returns all of the constraints attached to the specified window
		  #If TargetMacOS
		    // @property(nonatomic, readonly) NSArray<__kindof NSLayoutConstraint *> *constraints;
		    Declare Function getConstraints Lib "Foundation" Selector "constraints" (obj As ptr) As Ptr
	...
	#tag Method, Flags = &h0, CompatibilityFlags = (TargetDesktop and (Target64Bit))
		Function ConstraintsForControl(extends screen as Window, ctl as RectControl) As SOSLayoutConstraint()
		  // Returns the constraints attributed to a particular control.
		  #If TargetMacOS
		    Dim rv() As SOSLayoutConstraint
		    Dim ca() As SOSLayoutConstraint = Screen.Constraints
		    ...
		    Dim cca() As SOSLayoutConstraint = ctl.Constraints
		    For i As Integer = 0 To UBound(cca)
		      If cca(i).FirstItem = Ptr(ctl.Handle) Then
		        rv.Add cca(i)
		      End If
		    Next
	...
	#tag Method, Flags = &h0, CompatibilityFlags = (TargetDesktop and (Target64Bit))
		Sub ConvertConstraintsForAllControls(extends view as Window, newPriority as Double)
		  // Converts all constraints on a particular DesktopWindow to SOSLayoutConstraints, changing them all to a particular priority
		  
		  #If TargetMacOS
		    For Each ctl As Object In view.controls
		      If ctl IsA RectControl Then
		        view.ConvertConstraintsForControl(RectControl(ctl), newPriority)
		      End If
		    Next
		  #EndIf
		End Sub
	...
	#tag Method, Flags = &h0, CompatibilityFlags = (TargetDesktop and (Target64Bit))
		Sub ConvertConstraintsForControl(extends view as Window, item as RectControl, newPriority as Double = 1000.0)
		  // Converts all constraints for a specific DesktopUIControl on a particular DesktopWindow to SOSLayoutConstraints, changing them all to a particular Priority
		  ...
		  #If TargetMacOS
		    ...
		    For i As Integer = UBound(ca) DownTo 0
		      If ca(i).FirstItem = Ptr(item.Handle) Then
		        Dim c As New SOSLayoutConstraint(ca(i).FirstItem, ca(i).FirstAttribute, ca(i).relation, ca(i).SecondItem, ca(i).SecondAttribute, ca(i).Multiplier, ca(i).Offset)
		        c.Identifier = ca(i).identifier
		        If c.Identifier = "" Then
		          c.Identifier = item.Name + "_" + AttributeName(ca(i).firstAttribute)
	...
	#tag Method, Flags = &h0, CompatibilityFlags = (TargetDesktop and (Target64Bit))
		Sub ConvertConstraintsForControl(extends view as Window, item as RectControl, newPriority as SOSConstraintKit.Priorities)
		  // Converts all constraints for a specific DesktopUIControl on a particular DesktopWindow to SOSLayoutConstraints, changing them all to a particular Priority
	...
	#tag Method, Flags = &h21, CompatibilityFlags = (TargetDesktop and (Target64Bit))
		Private Sub ConvertControlToAutolayout(ctl as RectControl, useLeadingTrailing as Boolean = False)
		  #If TargetMacOS
		    // @property(copy) NSArray<__kindof NSView *> *subviews;
		    Declare Function getSubviews Lib "Foundation" Selector "subviews" (obj As ptr) As Ptr
		    Declare Sub setSubviews Lib "Foundation" Selector "setSubviews:" (obj As ptr, value As Ptr)
		    ...
		    Dim view As ptr = ptr(ctl.Handle)
		    Dim viewRect As NSRect = getFrame(view)
	...
	#tag Method, Flags = &h0, CompatibilityFlags = (TargetDesktop and (Target64Bit))
		Sub ConvertToAutolayout(Extends w as Window, useLeadingTrailing as Boolean = False, unconstrainWindow as boolean = False)
		  // Converts the DesktopWindow layout to one that's using constraints
	...
	#tag Method, Flags = &h0, CompatibilityFlags = (TargetDesktop and (Target64Bit))
		Function DirectionalLayoutMargins(Extends view as RectControl) As NSDirectionalEdgeInsets
		  #If TargetMacOS
		    // @property(nonatomic) NSDirectionalEdgeInsets directionalLayoutMargins;
		    Declare Function getDirectionalLayoutMargins Lib "Foundation" Selector "directionalLayoutMargins" (obj As integer) As NSDirectionalEdgeInsets
	...
	#tag Method, Flags = &h0, CompatibilityFlags = (TargetDesktop and (Target64Bit))
		Sub DirectionalLayoutMargins(Extends view as RectControl, assigns value as NSDirectionalEdgeInsets)
		  #If TargetMacOS
		    // @property(nonatomic) NSDirectionalEdgeInsets directionalLayoutMargins;
		    Declare Sub setDirectionalLayoutMargins Lib "Foundation" Selector "setDirectionalLayoutMargins:" (obj As integer, value As NSDirectionalEdgeInsets)
	...
	#tag Method, Flags = &h0, CompatibilityFlags = (TargetDesktop and (Target64Bit))
		Function DirectionalLayoutMargins(Extends view as Window) As NSDirectionalEdgeInsets
		  #If TargetMacOS
		    // @property(nonatomic) NSDirectionalEdgeInsets directionalLayoutMargins;
		    Declare Function getDirectionalLayoutMargins Lib "Foundation" Selector "directionalLayoutMargins" (obj As integer) As NSDirectionalEdgeInsets
	...
	#tag Method, Flags = &h0, CompatibilityFlags = (TargetDesktop and (Target64Bit))
		Sub DirectionalLayoutMargins(Extends view as Window, assigns value as NSDirectionalEdgeInsets)
		  #If TargetMacOS
		    // @property(nonatomic) NSDirectionalEdgeInsets directionalLayoutMargins;
		    Declare Sub setDirectionalLayoutMargins Lib "Foundation" Selector "setDirectionalLayoutMargins:" (obj As integer, value As NSDirectionalEdgeInsets)
	...
	#tag Method, Flags = &h0, CompatibilityFlags = (TargetDesktop and (Target64Bit))
		Function FirstBaselineOffset(extends view as RectControl) As Double
		  #If TargetMacOS
		    If Not (view IsA Label Or view IsA TextField Or view IsA TextArea) Then
		      Raise New UnsupportedOperationException("The passed control is not a text control")
		    End If
		    // @property(nullable, nonatomic, strong) UIFont *font;
		    Declare Function getFont Lib "Foundation" Selector "font" (obj As Integer) As Ptr
	...
	#tag Method, Flags = &h0, CompatibilityFlags = (TargetDesktop and (Target64Bit))
		Function HuggingPriorityForAxis(Extends item as ContainerControl, axis as SOSConstraintKit.Axis) As Double
		  #If TargetMacOS
		    
		    // - (NSLayoutPriority)contentHuggingPriorityForOrientation:(NSLayoutConstraintOrientation)orientation;
		    Declare Function contentHuggingPriorityForOrientation Lib "Foundation" Selector "contentHuggingPriorityForOrientation:" ( obj As integer , orientation As Integer ) As Single
	...
	#tag Method, Flags = &h0, CompatibilityFlags = (TargetDesktop and (Target64Bit))
		Sub HuggingPriorityForAxis(Extends item as ContainerControl, Axis as SOSConstraintKit.Axis, assigns priority as Double)
		  #If TargetMacOS
		    // - (void)setContentHuggingPriority:(NSLayoutPriority)priority forOrientation:(NSLayoutConstraintOrientation)orientation;
		    Declare Sub setContentHuggingPriority_forOrientation Lib "Foundation" Selector "setContentHuggingPriority:forOrientation:" ( obj As integer , priority As Single , orientation As Integer )
	...
	#tag Method, Flags = &h0, CompatibilityFlags = (TargetDesktop and (Target64Bit))
		Function HuggingPriorityForAxis(Extends item as RectControl, axis as SOSConstraintKit.Axis) As Double
		  #If TargetMacOS
	...
	#tag Method, Flags = &h0, CompatibilityFlags = (TargetDesktop and (Target64Bit))
		Sub HuggingPriorityForAxis(Extends item as RectControl, Axis as SOSConstraintKit.Axis, assigns priority as Double)
		  #If TargetMacOS
		    // - (void)setContentHuggingPriority:(NSLayoutPriority)priority forOrientation:(NSLayoutConstraintOrientation)orientation;
		    Declare Sub setContentHuggingPriority_forOrientation Lib "Foundation" Selector "setContentHuggingPriority:forOrientation:" ( obj As integer , priority As Single , orientation As Integer )
	...
	#tag Method, Flags = &h0, CompatibilityFlags = (TargetDesktop and (Target64Bit))
		Function InsetsLayoutMarginsFromSafeArea(extends view as ContainerControl) As Boolean
		  ...
		  #If TargetMacOS
		    ...
		    Declare Function getInsetsLayoutMarginsFromSafeArea Lib "Foundation" Selector "insetsLayoutMarginsFromSafeArea" (obj As integer) As Boolean
	...
	#tag Method, Flags = &h0, CompatibilityFlags = (TargetDesktop and (Target64Bit))
		Sub InsetsLayoutMarginsFromSafeArea(extends view as ContainerControl, assigns value as Boolean)
		  // Sets if the control is insetting layout margins from the safe area
		  #If TargetMacOS
		    
		    // @property(nonatomic) BOOL insetsLayoutMarginsFromSafeArea;
		    Declare Sub setInsetsLayoutMarginsFromSafeArea Lib "Foundation" Selector "setInsetsLayoutMarginsFromSafeArea:" (obj As integer, value As Boolean)
	...
	#tag Method, Flags = &h0, CompatibilityFlags = (TargetDesktop and (Target64Bit))
		Function InsetsLayoutMarginsFromSafeArea(extends view as RectControl) As Boolean
		  // Returns true if the control is insetting layout margins from the safe area
		  #If TargetMacOS
		    // @property(nonatomic) BOOL insetsLayoutMarginsFromSafeArea;
		    Declare Function getInsetsLayoutMarginsFromSafeArea Lib "Foundation" Selector "insetsLayoutMarginsFromSafeArea" (obj As integer) As Boolean
	...
	#tag Method, Flags = &h0, CompatibilityFlags = (TargetDesktop and (Target64Bit))
		Sub InsetsLayoutMarginsFromSafeArea(extends view as RectControl, assigns value as Boolean)
		  // Sets if the control is insetting layout margins from the safe area
		  #If TargetMacOS
	...
	#tag Method, Flags = &h0, CompatibilityFlags = (TargetDesktop and (Target64Bit))
		Function IntrinsicContentSize(extends view as ContainerControl) As Size
		  // Returns the instrinsic content size for the control
		  #If TargetMacOS
		    // @property(nonatomic, readonly) CGSize intrinsicContentSize;
		    Declare Function getIntrinsicContentSize Lib "Foundation" Selector "intrinsicContentSize" (obj As integer) As CGSize
	...
	#tag Method, Flags = &h0, CompatibilityFlags = (TargetDesktop and (Target64Bit))
		Function IntrinsicContentSize(extends view as RectControl) As Size
		  // Returns the instrinsic content size for the container
		  #If TargetMacOS
		    // @property(nonatomic, readonly) CGSize intrinsicContentSize;
		    Declare Function getIntrinsicContentSize Lib "Foundation" Selector "intrinsicContentSize" (obj As integer) As CGSize
	...
	#tag Method, Flags = &h0, CompatibilityFlags = (TargetDesktop and (Target64Bit))
		Sub InvalidateIntrinsicContentSize(extends view as ContainerControl)
		  ...
		  #If TargetMacOS
		    ...
		    invalidateIntrinsicContentSize(ptr(view.Handle))
		  #EndIf
		End Sub
	#tag EndMethod
	...
	#tag Method, Flags = &h0, CompatibilityFlags = (TargetDesktop and (Target64Bit))
		Sub InvalidateIntrinsicContentSize(extends view as RectControl)
		  // Asks the OS to recalculate the control's intrinsic content size
		  #If TargetMacOS
		    // - (void)invalidateIntrinsicContentSize;
		    Declare Sub invalidateIntrinsicContentSize Lib "Foundation" Selector "invalidateIntrinsicContentSize" (obj As ptr)
	...
	#tag Method, Flags = &h0, CompatibilityFlags = (TargetDesktop and (Target64Bit))
		Sub LayoutIfNeeded(extends view as Window)
		  // Asks the OS to update the window's layout if necessary.
		  #If TargetMacOS
		    // - (void)layoutIfNeeded;
		    Declare Sub layoutIfNeeded Lib "Foundation" Selector "layoutIfNeeded" (obj As integer)
	...
	#tag Method, Flags = &h0, CompatibilityFlags = (TargetDesktop and (Target64Bit))
		Function LayoutMargins(extends view as RectControl) As UIEdgeInsets
		  // Gets the Layout Margins for the control.
		  #If TargetMacOS
		    // @property(nonatomic) UIEdgeInsets layoutMargins;
		    Declare Function getLayoutMargins Lib "Foundation" Selector "layoutMargins" (obj As integer) As UIEdgeInsets
	...
	#tag Method, Flags = &h0, CompatibilityFlags = (TargetDesktop and (Target64Bit))
		Sub LayoutMargins(extends view as RectControl, assigns value as UIEdgeInsets)
		  // Sets the Layout Margins for the control.
		  #If TargetMacOS
		    // @property(nonatomic) UIEdgeInsets layoutMargins;
		    Declare Sub setLayoutMargins Lib "Foundation" Selector "setLayoutMargins:" (obj As integer, value As UIEdgeInsets)
	...
	#tag Method, Flags = &h0, CompatibilityFlags = (TargetDesktop and (Target64Bit))
		Function LayoutMargins(extends view as Window) As UIEdgeInsets
		  // Gets the Layout Margins for the window.
	...
	#tag Method, Flags = &h0, CompatibilityFlags = (TargetDesktop and (Target64Bit))
		Sub LayoutMargins(extends view as Window, assigns value as UIEdgeInsets)
		  // Gets the Layout Margins for the window.
	...
	#tag Method, Flags = &h0, CompatibilityFlags = (TargetDesktop and (Target64Bit))
		Function PreservesSuperviewLayoutMargins(extends view as ContainerControl) As Boolean
		  ...
		  #If TargetMacOS
		    ...
		    Declare Function getPreservesSuperviewLayoutMargins Lib "Foundation" Selector "preservesSuperviewLayoutMargins" (obj As integer) As Boolean
	...
	#tag Method, Flags = &h0, CompatibilityFlags = (TargetDesktop and (Target64Bit))
		Sub PreservesSuperviewLayoutMargins(extends view as ContainerControl, assigns value as boolean)
		  // Set to True if the container should preserve its superview's layout margins
		  #If TargetMacOS
		    
		    // @property(nonatomic) BOOL preservesSuperviewLayoutMargins;
		    Declare Sub setPreservesSuperviewLayoutMargins Lib "Foundation" Selector "setPreservesSuperviewLayoutMargins:" (obj As integer, value As Boolean)
	...
	#tag Method, Flags = &h0, CompatibilityFlags = (TargetDesktop and (Target64Bit))
		Function PreservesSuperviewLayoutMargins(extends view as RectControl) As Boolean
		  // Returns True if the control preserves its superview's layout margins
		  #If TargetMacOS
		    // @property(nonatomic) BOOL preservesSuperviewLayoutMargins;
		    Declare Function getPreservesSuperviewLayoutMargins Lib "Foundation" Selector "preservesSuperviewLayoutMargins" (obj As integer) As Boolean
	...
	#tag Method, Flags = &h0, CompatibilityFlags = (TargetDesktop and (Target64Bit))
		Sub PreservesSuperviewLayoutMargins(extends view as RectControl, assigns value as boolean)
		  // Set to True if the control should preserve its superview's layout margins
		  #If TargetMacOS
	...
	#tag Method, Flags = &h0, CompatibilityFlags = (TargetDesktop and (Target64Bit))
		Sub RecreateConstraintsForAllControls(extends view as Window)
		  // Recreates constrains for all controls on the view
		  #If TargetMacOS
		    Dim ca() As SOSConstraintKit.SOSLayoutConstraint = view.constraints
	...
	#tag Method, Flags = &h0, CompatibilityFlags = (TargetDesktop and (Target64Bit))
		Sub RecreateConstraintsForAllControls(extends view As Window, newPriority As Double = 1000.0)
		  // Recreates constrains for all controls on the view, with a specific priority
		  #If TargetMacOS
		    Dim ca() As SOSConstraintKit.SOSLayoutConstraint = view.constraints
	...
	#tag Method, Flags = &h0, CompatibilityFlags = (TargetDesktop and (Target64Bit))
		Sub RemoveConstraint(extends control as RectControl, constraint as SOSLayoutConstraint)
		  // Removes the specified constraint from the control
		  #If TargetMacOS
		    // Bail if the constraint is nil because the app will crash
		    If constraint = Nil Then
		    ...
		    #If TargetMacOS
		      // - (void)removeConstraint:(NSLayoutConstraint *)constraint;
		      Declare Sub removeConstraint Lib "Foundation" Selector "removeConstraint:" ( obj As integer , constraint As Ptr )
	...
	#tag Method, Flags = &h0, CompatibilityFlags = (TargetDesktop and (Target64Bit))
		Sub RemoveConstraint(extends screen as Window, constraint as SOSLayoutConstraint)
		  // Removes the specified constraint from the window
		  #If TargetMacOS
		    // Bail if the constraint is nil because the app will crash
		    If constraint = Nil Then
	...
	#tag Method, Flags = &h0, CompatibilityFlags = (TargetDesktop and (Target64Bit))
		Sub RemoveConstraintsForControl(extends view as ContainerControl, ctl as RectControl)
	...
	#tag Method, Flags = &h0, CompatibilityFlags = (TargetDesktop and (Target64Bit))
		Sub RemoveConstraintsForControl(extends view as Window, ctl as RectControl)
		  // Removes constraints for a control from a particular window
		  #If TargetMacOS
		    Dim ca() As SOSLayoutConstraint = view.constraints
	...
	#tag Method, Flags = &h0, CompatibilityFlags = (TargetDesktop and (Target64Bit))
		Function SafeAreaInsets(view as Window) As UIEdgeInsets
		  // Returns the safe area insets for the specified window
		  #If TargetMacOS
		    // @property(nonatomic, readonly) UIEdgeInsets safeAreaInsets;
		    Declare Function getSafeAreaInsets Lib "Foundation" Selector "safeAreaInsets" (obj As integer) As UIEdgeInsets
	...
	#tag Method, Flags = &h0, CompatibilityFlags = (TargetDesktop and (Target64Bit))
		Sub UpdateConstraints(extends control as RectControl)
		  // Asks the OS to update the constraints for a particular control.
		  #If TargetMacOS
		    // - (void)updateConstraintsIfNeeded;
		    Declare Sub updateConstraintsIfNeeded Lib "Foundation" Selector "updateConstraintsIfNeeded" (obj As integer)
	...
	#tag Method, Flags = &h0, CompatibilityFlags = (TargetDesktop and (Target64Bit))
		Sub UpdateConstraints(extends screen as Window)
		  // Asks the OS to update the constraints for a particular window.
		  #If TargetMacOS
		    // - (void)updateConstraintsIfNeeded;
		    Declare Sub updateConstraintsIfNeeded Lib "Foundation" Selector "updateConstraintsIfNeeded" (obj As integer)
	...
	#tag Method, Flags = &h0, CompatibilityFlags = (TargetDesktop and (Target64Bit))
		Sub UseIntrinsicWidth(extends ctl as RectControl)
		  // Removes the width constraint(s) so the control will use the intrinsic size
		  
		  #If TargetMacOS
		    Dim ca() As SOSLayoutConstraint = ctl.Constraints
		    For i As Integer = 0 To UBound(ca)
		      Dim ctlHandle As Ptr = Ptr(ctl.Handle)
		      
		      If ca(i).FirstItem = ctlHandle And ca(i).FirstAttribute = SOSLayoutConstraint.LayoutAttributes.Width And ca(i).SecondItem = Nil Then
		        ca(i).Active = False
		      End If
		    Next
	...
	#tag Method, Flags = &h21, CompatibilityFlags = (TargetDesktop and (Target64Bit))
		Private Function WindowToView(w as Window) As Ptr
		  Dim p As ptr = ptr(w.Handle)
		  #If TargetMacOS
		    If Not w IsA DesktopContainer Then
		      // @property(strong) __kindof NSView *contentView;
		      Declare Function getContentView Lib "Foundation" Selector "contentView" (obj As integer) As Ptr
		      p = getContentView(w.Handle)
		    End If
		  #EndIf
